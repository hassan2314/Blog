...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    try {
      ...
      const recentPosts = allPosts.documents.filter(post => 
        new Date(post.$createdAt) > lastMonth
      );
      
      const recentUsers = usersWithRoles.filter(user => 
      ...
      setStats({
        ...
        analytics: {
          ...
          growthRate: calculateGrowthRate(recentPosts.length, allPosts.documents.length),
  ...
  const adminCards = [
    ...
    },
    {
      ...
      color: 'bg-indigo-500',
      count: Object.values(stats.systemHealth).filter(s => s === 'healthy').length,
      subtext: `of ${Object.keys(stats.systemHealth).length} services`,
      trend: Object.values(stats.systemHealth).every(s => s === 'healthy') ? 'up' : 'warning'
    },
    {
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(stats.systemHealth).map(([service, status]) => (
                <div key={service} className={`p-4 rounded-lg ${getHealthColor(status)}`}>
                  <div className="flex items-center justify-between">
                  ...
                  <Link to={card.link} className="block p-6">
                    <div className="flex items-center">
                      ...
                      <div className="text-right">
                        <div className="flex items-center">
                          <div className="text-2xl font-bold text-gray-900 mr-2">{card.count}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white shadow rounded-lg">
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500">Current role: {userRole?.replace('_', ' ')}</p>
            </div>
            <div className="p-6">
              <div className="max-h-96 overflow-y-auto">
                ...
                {permissions.length === 0 && (
                  <p className="text-gray-500 text-sm text-center py-4">No permissions assigned</p>
                )}
              </div>
            </div>
          </div>
